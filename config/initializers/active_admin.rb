...
ActiveAdmin.setup do |config|
  ...
  #
  config.site_title = "Djoulde Transports — Admin"
  ...
  #
  # Ticket 12: there is no separate AdminUser model. Admins are Devise `User`
  # records carrying the `super_admin` Rolify role, signed in via cookie
  # session at /users/sign_in. `authenticate_admin!` / `current_admin` are
  ...
  #
  # Ticket 12: log out through the shared Devise User session. Active Admin v4
  # always renders this link as a DELETE, which matches Devise's route.
