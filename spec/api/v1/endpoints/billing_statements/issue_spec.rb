...
RSpec.describe API::V1::Endpoints::BillingStatements::Issue do
  subject(:do_request) do
    patch "/api/v1/billing_statements/#{statement_id}/issue", params: params, headers: headers
  end
  ...
  context "when the user is an admin" do
    ...
    let(:params)  { {issued_on: (month.next_month + 2.days).to_s} }

    context "with a draft statement" do
      before { do_request }
      ...
      it "flips the status to issued" do
        expect(statement.reload.status).to eq("issued")
      end

      it "stamps issued_on" do
        expect(statement.reload.issued_on.to_s).to eq((month.next_month + 2.days).to_s)
      end
    end
