...
RSpec.describe Tank do
  ...
  let(:tank)  { described_class.new(truck: truck, plate_number: "TK-1", capacity_liters: 30_000) }
  ...
  it "is valid with a truck, plate_number, and capacity_liters" do
  ...
  it "requires capacity_liters" do
    tank.capacity_liters = nil
    tank.validate
    expect(tank.errors[:capacity_liters]).to be_present
  end

  it "rejects a second tank on the same truck" do
    described_class.create!(truck: truck, plate_number: "TK-A", capacity_liters: 25_000)
    duplicate = described_class.new(truck: truck, plate_number: "TK-B", capacity_liters: 25_000)
    duplicate.validate
    expect(duplicate.errors[:truck_id]).to be_present
  end

  it "enforces case-insensitive plate_number uniqueness" do
    described_class.create!(truck: truck, plate_number: "TK-9", capacity_liters: 25_000)
    other_truck = Truck.create!(plate_number: "HEAD-X")
    duplicate = described_class.new(truck: other_truck, plate_number: "tk-9", capacity_liters: 25_000)
    duplicate.validate
